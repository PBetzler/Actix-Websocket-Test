...
pub struct Lobby {
    ...
    pub room: HashSet<Uuid>,      //room id  to list of users id
}
...
impl Handler<ClientActorMessage> for Lobby {
    ...
    fn handle(&mut self, msg: ClientActorMessage, _ctx: &mut Context<Self>) -> Self::Result {
        if msg.msg.starts_with("\\w") {
            ...
            self.room.iter().for_each(|client| self.send_message(&msg.msg, client));
        }
    }
}
...
impl Handler<WsMessage> for Lobby {
    ...
    fn handle(&mut self, msg: WsMessage, _ctx: &mut Context<Self>) -> Self::Result {
        self.room.iter().for_each(|client| self.send_message(&msg.0, client));
    }
}